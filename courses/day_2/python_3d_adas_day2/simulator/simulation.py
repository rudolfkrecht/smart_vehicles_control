...
class Simulation:
    ...
    def _preview_distance(self, speed: float) -> float:
        """Read the controller's requested look-ahead distance safely."""
    ...
    def metrics(self) -> dict[str, float | None]:
        if not self.history:
            return {
                ...
                "mean_absolute_cross_track_error_m": None,
                "outside_road_percent": None,
                "rms_steering_rate_degrees_s": None,
                "peak_lateral_acceleration_mps2": None,
        ...
        for previous, current in zip(self.history, self.history[1:]):
            ...
            if step_dt > 0.0:
                steering_rates.append(
                    (
                        float(current["steering_rad"])
                        - float(previous["steering_rad"])
                    )
                    / step_dt
                )
        rms_steering_rate = (
            math.degrees(
                math.sqrt(
                    sum(rate * rate for rate in steering_rates)
                    / len(steering_rates)
                )
            )
            if steering_rates
        ...
        lateral_accelerations = [
            float(row["speed_mps"]) ** 2
            / self.vehicle.parameters.wheelbase
            * math.tan(float(row["steering_rad"]))
        ...
        return {
            ...
            "mean_absolute_cross_track_error_m": mean_cte,
            "outside_road_percent": outside_road,
            "rms_steering_rate_degrees_s": rms_steering_rate,
            "peak_lateral_acceleration_mps2": max(
                abs(value) for value in lateral_accelerations
            ),
