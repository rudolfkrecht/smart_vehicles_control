"""Vehicle dynamics and public controller data types for Day 4."""
...
class ControlCommand:
    """Actuator commands plus controller metadata used by the dashboard."""

    throttle: float = 0.0
    brake: float = 0.0
    steering: float = 0.0
    selected_target_speed: float | None = None
    desired_gap: float | None = None
    mode: str = "CRUISE"
    ...
    def limited(self, max_steering: float) -> "ControlCommand":
        ...
        return ControlCommand(
            ...
            selected_target_speed=self.selected_target_speed,
            desired_gap=self.desired_gap,
            mode=str(self.mode),
            supervisor_active=bool(self.supervisor_active),
        )


@dataclass(frozen=True)
class Observation:
    """Measurements available to the student controller at one time step."""
    ...
    sensor_range: float
    range_sensor_healthy: bool
    range_measurement_age: float
    active_fault: str
    ...
    def step(
        ...
        *,
        ...
        requested_force = (
            ...
            - command.brake
            * p.maximum_brake_force
            * clamp(braking_efficiency, 0.0, 1.0)
        )
