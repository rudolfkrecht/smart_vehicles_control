"""Day 4 simulation orchestration, fault injection, logging and metrics."""
...
from .faults import FaultScenario, get_scenario
...
def lead_speed_schedule(time_s: float) -> float:
    """Deterministic stop-and-go traffic retained for Day 4 evaluation."""
...
class Simulation:
    def __init__(
        ...
        *,
        ...
        scenario: str | FaultScenario | None = None,
    ) -> None:
        ...
        self.lateral_push_applied = False
        self.reset()
    ...
    def observe(self, dt: float) -> Observation:
        ...
        range_sensor_healthy = self.scenario.radar_is_healthy(self.time)
        range_measurement_age = self.scenario.radar_age(self.time)
        detected = (
            range_sensor_healthy
            and 0.0 < true_gap <= self.sensor_range
        )
        ...
        return Observation(
            ...
            range_sensor_healthy=range_sensor_healthy,
            range_measurement_age=range_measurement_age,
            active_fault=self.scenario.active_fault(self.time),
        )

    def _apply_lateral_push_if_due(self, dt: float) -> bool:
        push_time = self.scenario.lateral_push_time_s
        if (
            self.lateral_push_applied
            or push_time is None
            or not (self.time <= push_time < self.time + dt)
        ):
            return False
        state = self.vehicle.state
        displacement = self.scenario.lateral_push_m
        state.x += -math.sin(state.heading) * displacement
        state.y += math.cos(state.heading) * displacement
        self.lateral_push_applied = True
        return True
    ...
    def step(
        ...
        lateral_push_applied_now = self._apply_lateral_push_if_due(dt)
        self.vehicle.step(
            command,
            before.slope_radians,
            dt,
            off_road=before.off_road,
            braking_efficiency=self.scenario.braking_efficiency,
        )
        ...
        self.history.append(
            {
                ...
                "mode": command.mode,
                "supervisor_active": command.supervisor_active,
                "range_sensor_healthy": after.range_sensor_healthy,
                "range_measurement_age_s": after.range_measurement_age,
                "active_fault": after.active_fault,
                "scenario": self.scenario.name,
                "braking_efficiency": self.scenario.braking_efficiency,
                "applied_brake": (
                    command.brake * self.scenario.braking_efficiency
                ),
                "lateral_push_applied": lateral_push_applied_now,
    ...
    def metrics(self) -> dict[str, float | int | None]:
        ...
        return {
            ...
            "safe_stop_samples": sum(
                str(row["mode"]) == "SAFE_STOP" for row in self.history
            ),
            "fault_samples": sum(
                str(row["active_fault"]) != "NONE" for row in self.history
            ),
        }
