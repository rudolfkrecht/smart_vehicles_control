"""Interactive PyQt laboratory for all six Day 3 lessons.

Run from the package root:

    python day_3/gui/day3_vehicle_simulator.py
"""
...
class RoadCanvas(QWidget):
    """Moving road view with planning, tracking and traffic overlays."""

    def __init__(self) -> None:
        super().__init__()
        self.simulation = IntegratedSimulation()
        self.snapshot: LiveSnapshot | None = None
        self.show_curvature = True
        self.show_geometry = True
        self.setMinimumSize(760, 390)

    def set_simulation(self, simulation: IntegratedSimulation) -> None:
    ...
    def _draw_hud(self, painter: QPainter) -> None:
        ...
        painter.setPen(QColor(NAVY))
        painter.drawText(28, 38, "DAY 3 — INTEGRATED ADAS LAB")
        ...
        if snapshot:
            lines = (
                f"Ego speed: {snapshot.vehicle.speed:4.1f} m/s",
                f"Selected target: {snapshot.selected_target_speed:4.1f} m/s",
                (
                    f"Gap: {snapshot.gap:4.1f} m"
                    if math.isfinite(snapshot.gap)
...
class ControlPanel(QWidget):
    def __init__(self, window: "Day3Window") -> None:
        super().__init__()
        self.window = window
        ...
        self.preset.addItems(
            [
                ...
                "Lesson 1 — excessive speed",
                "Lesson 2 — curvature map",
                "Lesson 3 — curve-aware speed",
        ...
        architecture = QGroupBox("Enabled layers")
        ...
        self.lead_preset.addItems(
            ["stop_and_go", "steady", "slow", "late_brake"]
        )
    ...
    def apply_preset(self, index: int) -> None:
        # Values correspond to the lesson demonstrations and student baseline.
        presets = [
            dict(curve=False, traffic=False, states=False, speed=10, lat=2.5, preview=14, headway=1.5, lead="steady"),
            dict(curve=False, traffic=False, states=False, speed=18, lat=4.5, preview=0, headway=1.5, lead="steady"),
            dict(curve=True, traffic=False, states=False, speed=15, lat=2.5, preview=14, headway=1.5, lead="steady"),
            dict(curve=True, traffic=False, states=False, speed=15, lat=2.5, preview=14, headway=1.5, lead="steady"),
            dict(curve=True, traffic=True, states=False, speed=15, lat=2.5, preview=14, headway=0.7, lead="stop_and_go"),
            dict(curve=True, traffic=True, states=False, speed=15, lat=2.5, preview=14, headway=1.5, lead="stop_and_go"),
            dict(curve=True, traffic=True, states=True, speed=15, lat=2.5, preview=14, headway=1.5, lead="late_brake"),
            dict(curve=True, traffic=True, states=True, speed=18, lat=4.5, preview=3, headway=0.65, lead="stop_and_go"),
            dict(curve=True, traffic=True, states=True, speed=15, lat=2.5, preview=14, headway=1.5, lead="stop_and_go"),
        ]
        values = presets[index]
        self.curve_speed.setChecked(values["curve"])
        self.traffic.setChecked(values["traffic"])
        self.state_machine.setChecked(values["states"])
        ...
        if index == 7:
            self.emergency_ttc.setValue(0.7)
            self.emergency_gap.setValue(1.2)
        else:
            self.emergency_ttc.setValue(1.25)
            self.emergency_gap.setValue(3.0)
        self.window.apply_controls()


class Day3Window(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle(
        ...
        self.lead_speed_trace = TraceBuffer()
        self.gap_trace = TraceBuffer()
        self.desired_gap_trace = TraceBuffer()
        self.accel_trace = TraceBuffer()
        self.lateral_trace = TraceBuffer()
        self.error_trace = TraceBuffer()
        self.minimum_gap = math.inf
        self.maximum_path_error = 0.0
        self.maximum_lateral_acceleration = 0.0
        ...
        metrics = QGridLayout()
        self.state_card = MetricCard("BEHAVIOUR")
        self.gap_card = MetricCard("MINIMUM GAP")
        self.path_card = MetricCard("MAX PATH ERROR")
        self.lateral_card = MetricCard("PEAK LATERAL ACCEL.")
        ...
        self.speed_plot = TracePlot(
            "Speed command arbitration",
            0.0,
            20.0,
            [
                ("ego", BLUE, self.speed_trace),
                ("selected", PURPLE, self.target_trace),
                ("lead", RED, self.lead_speed_trace),
            ],
        )
        self.gap_plot = TracePlot(
            ...
            55.0,
            [
                ("actual gap", BLUE, self.gap_trace),
                ("desired", AMBER, self.desired_gap_trace),
            ],
        )
        self.accel_plot = TracePlot(
            ...
            6.0,
            [
                ("longitudinal", BLUE, self.accel_trace),
                ("lateral", RED, self.lateral_trace),
            ],
        )
    ...
    def build_scenario(self) -> IntegratedScenario:
    ...
    def apply_controls(self) -> None:
        ...
        self.simulation = IntegratedSimulation(self.build_scenario())
    ...
    def clear_traces(self) -> None:
        for buffer in (
            self.speed_trace,
            self.target_trace,
            self.lead_speed_trace,
            self.gap_trace,
            self.desired_gap_trace,
            self.accel_trace,
            self.lateral_trace,
            self.error_trace,
        ):
    ...
    def capture(self, snapshot: LiveSnapshot) -> None:
        self.speed_trace.append(snapshot.vehicle.speed)
        self.target_trace.append(snapshot.selected_target_speed)
        self.lead_speed_trace.append(snapshot.lead_speed)
        if math.isfinite(snapshot.gap):
            self.gap_trace.append(snapshot.gap)
            self.desired_gap_trace.append(snapshot.traffic.desired_gap)
            self.minimum_gap = min(self.minimum_gap, snapshot.gap)
        self.accel_trace.append(
            snapshot.longitudinal.applied_acceleration
        )
        self.lateral_trace.append(snapshot.lateral_acceleration)
        self.error_trace.append(snapshot.cross_track_error)
        self.maximum_path_error = max(
            self.maximum_path_error,
            abs(snapshot.cross_track_error),
        )
        self.maximum_lateral_acceleration = max(
            self.maximum_lateral_acceleration,
            abs(snapshot.lateral_acceleration),
        )
        self.collision_seen = self.collision_seen or snapshot.collision

    def update_cards(self, snapshot: LiveSnapshot) -> None:
        state_color = STATE_COLORS[snapshot.traffic.state]
        self.state_card.set_value(snapshot.traffic.state.value, state_color)
        ...
        lateral_color = (
            GREEN
            if self.maximum_lateral_acceleration
            <= self.panel.max_lateral.value() + 0.4
            else AMBER
        )
        self.lateral_card.set_value(
            f"{self.maximum_lateral_acceleration:.2f} m/s²",
            lateral_color,
        )
