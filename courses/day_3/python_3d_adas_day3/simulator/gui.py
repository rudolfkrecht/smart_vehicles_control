"""Tkinter application for the cumulative Day 3 ACC simulator."""
...
class SimulatorApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Python 3D ADAS simulator — Day 3 adaptive cruise control")
    ...
    def _draw_dashboard(self, height: int) -> None:
        ...
        self.canvas.create_text(
            ...
            panel_y + 12,
            anchor="nw",
            text=f"DAY 3 ACC LAB  •  {self.controller_name.get().upper()}",
    ...
    def save_csv(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            title="Save Day 3 experiment log",
            defaultextension=".csv",
            filetypes=(("CSV files", "*.csv"), ("All files", "*.*")),
            initialfile="day3_acc_experiment.csv",
        )
        if not path:
            return
        try:
