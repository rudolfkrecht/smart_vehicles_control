"""Tkinter application for the lightweight 3D simulator."""
...
class SimulatorApp:
    ...
    def _build_toolbar(self) -> None:
        ...
        self.target_label = ttk.Label(toolbar, text="15.0 m/s", width=9)
    ...
    def _draw(self) -> None:
        ...
        self._draw_speed_plot(width, height)
    ...
    def _draw_dashboard(self, width: int, height: int) -> None:
        ...
        self.canvas.create_text(
            ...
            panel_y + 12,
            anchor="nw",
            text=f"3D ADAS LAB  •  {self.controller_name.get().upper()}",
            fill="#7fd1ff",
    ...
    def _draw_speed_plot(self, width: int, height: int) -> None:
        plot_w = 360
        plot_h = 118
        x0 = width - plot_w - 18
        y0 = height - plot_h - 18
        ...
        max_speed = max(
            5.0,
            float(self.target_speed.get()) * 1.25,
            max(float(row["speed_mps"]) for row in history) * 1.05,
        )

        def point(index: int, speed: float) -> tuple[float, float]:
            return (
                x0 + 8 + index / max(1, len(history) - 1) * (plot_w - 16),
                y0 + plot_h - 8 - speed / max_speed * (plot_h - 22),
            )

        target_points = [
            coordinate
            for index, row in enumerate(history)
            for coordinate in point(index, float(row["target_speed_mps"]))
        ]
        speed_points = [
            coordinate
            for index, row in enumerate(history)
            for coordinate in point(index, float(row["speed_mps"]))
        ]
        self.canvas.create_line(
            *target_points,
            fill="#f7c948",
        ...
        self.canvas.create_line(*speed_points, fill="#55b7ff", width=2)
        self.canvas.create_text(
            x0 + 10,
            y0 + 8,
            anchor="nw",
            text="SPEED HISTORY  — target  — actual",
            fill="#ffffff",
            font=("TkDefaultFont", 9, "bold"),
        )
