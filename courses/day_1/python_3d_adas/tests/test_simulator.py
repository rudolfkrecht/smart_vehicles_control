...
class PipelineTests(unittest.TestCase):
    def test_reference_controller_tracks_speed_and_lane(self) -> None:
        simulation = Simulation(ReferenceController(), target_speed=15.0)
        simulation.run(90.0)
        metrics = simulation.metrics()
        self.assertIsNotNone(metrics["rise_time_s"])
        self.assertLess(float(metrics["rise_time_s"]), 10.0)
        self.assertLess(float(metrics["overshoot_percent"]), 10.0)
        self.assertLess(abs(float(metrics["final_error_mps"])), 0.25)
